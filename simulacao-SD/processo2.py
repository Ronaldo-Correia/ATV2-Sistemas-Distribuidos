...
PORTA = 5002
VIZINHOS = [5001, 5003]  # processo1 e processo3
clock = 0
lock = threading.Lock()
...
def atualizar_relogio(recebido):
    ...
    with lock:
        ...
        log(f"[P2] Mensagem recebida. Relógio atualizado para {clock}")
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    with lock:
                        ...
                        log(f"[P2] Enviou para P{porta-5000} com clock {clock}")
            except Exception as e:
                log(f"[P2] Falha ao enviar para porta {porta}: {e}")
